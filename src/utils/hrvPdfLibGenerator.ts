 ...
 */
export async function generateFilledPO(templateUrl: string, orderData: HRVOrderData): Promise<Blob> {
...
async function extractKeywordPositions(page: any): Promise<Record<string, { x: number; y: number }>> {
  ...
  // based on the HRV PO FORMAT template layout
...
export async function downloadFilledPO(templateUrl: string, orderData: HRVOrderData, filename?: string): Promise<void> {
...
export async function previewFilledPO(templateUrl: string, orderData: HRVOrderData): Promise<string> {
  const blob = await generateFilledPO(templateUrl, orderData);
